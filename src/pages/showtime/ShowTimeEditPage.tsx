...
const ShowTimeEditDTO = Yup.object({
  ...
  screen: Yup.string()
    .oneOf(["SCREEN1", "SCREEN2", "SCREEN3"], "Invalid Screen")
    .required(),
  price: Yup.number()
...
const ShowTimeEditPage = () => {
  ...
  const submitForm = async (data: any) => {
    try {
      const res: any = await showtimeService.getRequest(
        `/showtime/getshowtime/${id}`,
      );
      ...
      await showtimeService.putRequest(`/showtime/${id}`, payload);
      ...
      const errorMessage =
        exception.response?.data?.message || "Check console for details";

      toast.error("Update Failed", {
        description:
          typeof serverError === "string" ? serverError : errorMessage,
      });
    }
  };
  ...
  const getShowTimeDetail = async () => {
    try {
      const response = await showtimeService.getRequest(`/showtime/${id}`);

      const movieFieldValue =
        typeof response.data.movieId === "object"
